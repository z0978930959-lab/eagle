...
function startRound(dg) {
  // v36：除了本局已出過的題（usedWords），連同這間房「前幾局」出過的題（seenWords）
  // 一起排除，避免同一組人連打好幾局老是遇到同樣的詞。
  const pick = drawOneWord([...(dg.seenWords || []), ...dg.usedWords]);
  dg.usedWords.push(pick.w);
  dg.seenWords = [...(dg.seenWords || []), pick.w].slice(-SEEN_MEMORY);
  dg.word = pick;
  dg.strokes = [];
  dg.guessLog = [];
  dg.lastResult = null;
...
export function createDrawGuessRoom({ code, rounds }) {
  ...
  const room = {
    ...
    dg: {
      ...
      word: null,
      usedWords: [],
      seenWords: [], // 跨局記憶：再玩一局時不清空，讓題目盡量不重複
      strokes: [],
      guessLog: [],
      deadline: null,
      lastResult: null,
...
export function actDgRematch(room, seat) {
  ...
  dg.score = 0;
  dg.usedWords = [];
  // 注意：seenWords 故意不清空——這正是跨局不重複的關鍵
  startRound(dg);
  pushLog(dg, '🔁 再玩一局！');
}
