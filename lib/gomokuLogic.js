 ...
 *
 * 唯一的額外規則（禁手）：
 *   ★ 先手（黑）不可下「雙活三」——一手同時做出兩個以上的活三就是禁手，
 *     後端直接擋下，前端也會把禁手點標成 ✕。白方沒有任何禁手。
 *     例外：那一手若本身就連成五子，五連優先，直接判勝。
 *
 * 兩種模式：
 *   普通模式：正常下，只擋禁手。
...
/**
 * 黑棋在 (x,y) 落子是否構成「雙活三」禁手。
 * 五連優先：那一手若直接連成五子就不算禁手。
 */
export function isForbidden(board, x, y) {
  if (board[idx(x, y)] !== 0) return false;
  board[idx(x, y)] = BLACK;
  let bad = false;
  if (!makesFive(board, x, y, BLACK)) {
    let threes = 0;
    for (const [dx, dy] of DIRS) {
      if (threatAt(board, x, y, BLACK, dx, dy) === 'three_open') threes++;
    }
    bad = threes >= 2;
  }
  board[idx(x, y)] = 0;
...
function forbiddenPoints(board) {
  ...
  for (let y = 0; y < SIZE; y++) {
    for (let x = 0; x < SIZE; x++) {
      if (board[idx(x, y)] === 0 && isForbidden(board, x, y)) out.push([x, y]);
    }
  }
  return out;
}
...
export function actGomokuPlace(room, role, payload) {
  ...
  const c = gm.turn;
  // ★ 禁手：黑方不可下雙活三（該手若直接成五則不受限）
  if (c === BLACK && isForbidden(gm.board, x, y)) throw new Error('FORBIDDEN_MOVE');
