 ...
 *
 *   ★ 黑方無合法著手 → 判黑方輸。輪到黑方時，若盤面上每一個空點都是禁手
 *     （典型情況是被逼到只能連六），黑方無路可走，該局直接判白方勝。
 *     這條在輪次交給黑方的當下就檢查，不必等黑方真的去點。
 *
 * 兩種模式：
 *   普通模式：正常下，只擋禁手。
...
function startGame(room) {
  ...
  room.gm = {
    ...
    threats: [],
    forbidden: [],
    drawOffer: null, // 和局要求：{ by: 'a'|'b', ts } —— 對方回應或自己再落子就撤銷
    log: [],
    seq: 0,
  };
...
export function actGomokuPlace(room, role, payload) {
  ...
  gm.moves += 1;
  gm.last = { x, y };
  // 落子代表不想談了：撤銷還沒被回應的和局要求，免得對方事後才按同意
  if (gm.drawOffer) {
    gm.drawOffer = null;
    pushLog(gm, '↩️ 有人繼續落子，和局要求自動撤銷');
  }
  ...
  gm.seq += 1;
  refreshAnalysis(room);
  checkBlackStuck(room); // 換手後若輪到黑方且無合法點 → 直接判負
}

/**
 * 輪到黑方時，若每一個空點都是禁手（被逼到只能連六之類），黑方無路可走 → 判負。
 * 必須在 refreshAnalysis 之後呼叫：那時 gm.forbidden 才是最新的禁手點清單。
 */
function checkBlackStuck(room) {
  const gm = room.gm;
  if (gm.phase !== 'playing' || gm.turn !== BLACK) return;
  const empties = SIZE * SIZE - gm.moves;
  if (empties === 0 || gm.forbidden.length < empties) return;

  gm.phase = 'gameover';
  gm.winner = other(gm.blackSeat);
  gm.stuck = true;
  const kinds = new Set(gm.forbidden.map(([, , k]) => k));
  const why = kinds.size === 1 && kinds.has('overline') ? '被迫連六' : '每一點都是禁手';
  pushLog(gm, `⛔ 黑方無合法著手（${why}）——判 ${seatName(gm.winner)} 勝`);
  settleGame(room);
  gm.seq += 1;
  refreshAnalysis(room);
}
...
/**
 * 提出和局要求。同時送出一張「投降輸一半」貼圖，對方畫面上會看到。
 * 已經有人提過而且還沒被回應時，不能重複提。
 */
export function actGomokuOfferDraw(room, role) {
  const gm = room.gm;
  if (!gm || gm.phase !== 'playing') throw new Error('WRONG_PHASE');
  if (gm.drawOffer) throw new Error('DRAW_PENDING');
  gm.drawOffer = { by: role, ts: Date.now() };
  putSticker(room, role, 'surrender');
  pushLog(gm, `🤝 ${seatName(role)} 提出和局要求`);
  gm.seq += 1;
}

export function actGomokuAnswerDraw(room, role, payload) {
  const gm = room.gm;
  if (!gm || gm.phase !== 'playing') throw new Error('WRONG_PHASE');
  const offer = gm.drawOffer;
  // 只有「被提議的那一方」能回應，提議者自己不能一個人拍板
  if (!offer || offer.by === role) throw new Error('NO_DRAW_OFFER');

  gm.drawOffer = null;
  if (payload?.accept) {
    gm.phase = 'gameover';
    gm.winner = null; // 和局：雙方都不加勝場
    pushLog(gm, '🤝 雙方同意和局');
    settleGame(room);
    refreshAnalysis(room);
  } else {
    pushLog(gm, `✋ ${seatName(role)} 婉拒和局，繼續下`);
  }
  gm.seq += 1;
}
...
const GM_STICKERS = ['taunt_1', 'taunt_2', 'surrender'];
...
export function gomokuViewFor(room, role) {
  ...
  return {
    ...
    // 和局要求：分成「我提的（等對方回）」與「對方提的（等我回）」
    drawOffer: gm.drawOffer
      ? { by: gm.drawOffer.by, mine: gm.drawOffer.by === role, forMe: gm.drawOffer.by !== role }
      : null,
    isDraw: gm.phase === 'gameover' && !gm.winner,
    stuck: !!gm.stuck,
    winner: gm.winner,
    iWon: gm.winner === role,
    winLine: gm.winLine,
