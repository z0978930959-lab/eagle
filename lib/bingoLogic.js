...
function freshPlayer() {
  return { options: genOptions(), board: null, rps: null, lines: 0 };
}

export function createBingoRoom({ code }) {
  return {
    code,
    type: 'bingo',
    status: 'waiting',
...
export function bingoViewFor(room, role) {
  ...
  return {
    ...
    turn: b.turn,
    called: b.called,
    myLines: me?.board ? countLines(me.board, calledSet) : 0,
    oppLines: opp?.board ? countLines(opp.board, calledSet) : 0,
    // 聽牌警示：「對方」聽牌時顯示給我看（聽幾張，不透露是哪幾張）
    oppTenpai: b.phase === 'play' && opp?.board ? tenpaiCount(opp.board, calledSet) : 0,
