...
// ------------------------------------------------------------------
/* 出手時機四區域（依投手體力縮放，前後端共用同一公式）
 * 回傳三個「半寬」（時機條半邊 0~50 的位置刻度）：
 *   perfect：★完美區半寬（體力 100% 時 7，越累越窄）
 *   good　 ：○不錯區外緣（100% 時 21）
 *   poor　 ：△勉強區外緣（100% 時 36，之外＝✕走鐘）
 * 體力 100→80：完美區逐步縮小，被不錯/勉強區取代（走鐘區不變）
 * 體力 <80　 ：勉強區外緣開始內縮 → 走鐘區隨之增加
 * 體力 <60　 ：走鐘區顯著遞增、完美區顯著遞減
 */
export function releaseWindows(stamina = 100) {
  const s = clamp(stamina, 0, 100);
  let perfect;
  let good;
  let poor;
  if (s >= 80) {
    const t = (100 - s) / 20; // 0→1
    perfect = 7 - 3 * t; // 7 → 4
    good = 21 - 4 * t; // 21 → 17
    poor = 36; // 走鐘區不變
  } else if (s >= 60) {
    const t = (80 - s) / 20; // 0→1
    perfect = 4 - 1.5 * t; // 4 → 2.5
    good = 17 - 3 * t; // 17 → 14
    poor = 36 - 6 * t; // 36 → 30（走鐘區開始擴大）
  } else {
    const t = Math.min(1, (60 - s) / 40); // 60→20 封頂
    perfect = Math.max(0.8, 2.5 - 1.7 * t); // 2.5 → 0.8（顯著遞減）
    good = Math.max(6, 14 - 8 * t); // 14 → 6
    poor = Math.max(12, 30 - 14 * t); // 30 → 12（走鐘區顯著遞增）
  }
  return { perfect, good, poor };
}

export function releaseZoneOf(release, stamina = 100) {
  if (release == null) return 'good'; // 相容舊客戶端：視為中性
  const w = releaseWindows(stamina);
...
// ------------------------------------------------------------------
export function buildFlightPath({ targetX, targetY, actualX, actualY, pitchTypeId, grade = 'B', effStuff = 50, eephus = false }) {
