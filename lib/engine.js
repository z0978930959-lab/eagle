...
// ------------------------------------------------------------------
// 出手時機四區域 → 落點偏移（新制）：
//   perfect（完美）：指哪投哪，落點＝玩家選的位置，一單位都不偏
//   good　（不錯）：微幅隨機偏移——投邊角可能剛好偏出帶外變壞球
//   poor　（勉強）：明顯偏移，邊角球風險大增
//   worst （走鐘）：失投——變成超慢的紅中小便球（eephus）
// 控球高可縮小 good/poor 的偏移量；陌生球種（B/C 級）偏移再加成。
// ------------------------------------------------------------------
/* 出手時機四區域（依「球種等級 × 投手體力」縮放，前後端共用同一公式）
 * 回傳三個「半寬」（時機條半邊 0~50 的位置刻度）：
 *   perfect：★完美區半寬｜good：○不錯區外緣｜poor：△勉強區外緣（之外＝✕走鐘）
 *
 * 球種等級決定滿體力時的基準（四段條寬 ★/○/△/✕，各為全條 %）：
 *   S：四區完全均等 25/25/25/25——拿手到閉著眼投都穩
 *   A：19/25/30/26——完美開始變窄、走鐘略增
 *   B：14/28/30/28（v9 原基準）
 *   C：10/26/30/34——完美很窄、走鐘明顯
 *
 * 體力再往下疊乘（對任何等級同比例）：
 *   100→80%：完美區逐步縮小，被不錯/勉強區取代（走鐘區不變）
 *   <80%　 ：勉強區外緣開始內縮 → 走鐘區隨之增加
 *   <60%　 ：走鐘區顯著遞增、完美區顯著遞減
 */
export const GRADE_RELEASE_BASE = {
  S: { perfect: 12.5, good: 25, poor: 37.5 },
  A: { perfect: 9.5, good: 22, poor: 37 },
  B: { perfect: 7, good: 21, poor: 36 },
  C: { perfect: 5, good: 18, poor: 33 },
};

// 體力縮放係數（相對滿體力基準的比例；以 v9 的 B 級絕對值換算，向下相容）
function staminaFactors(s) {
  if (s >= 80) {
    const t = (100 - s) / 20;
    return { p: 1 - (3 / 7) * t, g: 1 - (4 / 21) * t, o: 1 };
  }
  if (s >= 60) {
    const t = (80 - s) / 20;
    return { p: 4 / 7 - (1.5 / 7) * t, g: 17 / 21 - (3 / 21) * t, o: 1 - (6 / 36) * t };
  }
  const t = Math.min(1, (60 - s) / 40);
  return {
    p: Math.max(0.8 / 7, 2.5 / 7 - (1.7 / 7) * t),
    g: Math.max(6 / 21, 14 / 21 - (8 / 21) * t),
    o: Math.max(12 / 36, 30 / 36 - (18 / 36) * t),
  };
}

export function releaseWindows(stamina = 100, grade = 'B') {
  const base = GRADE_RELEASE_BASE[grade] || GRADE_RELEASE_BASE.B;
  const f = staminaFactors(clamp(stamina, 0, 100));
  return { perfect: base.perfect * f.p, good: base.good * f.g, poor: base.poor * f.o };
}

export function releaseZoneOf(release, stamina = 100, grade = 'B') {
  if (release == null) return 'good'; // 相容舊客戶端：視為中性
  const w = releaseWindows(stamina, grade);
  // 光標分數 = 100 - 2×|距中線|，故半寬 w 對應門檻分數 100 - 2w
  if (release >= 100 - 2 * w.perfect) return 'perfect';
  if (release >= 100 - 2 * w.good) return 'good';
  if (release >= 100 - 2 * w.poor) return 'poor';
  return 'worst';
}

export function releaseOffset({ zone, control = 50, grade = 'B' }) {
  if (zone === 'perfect') return { dx: 0, dy: 0 };
  const gradeExtra = { S: 0, A: 0.5, B: 2, C: 4 }[grade] ?? 2;
  const ctlScale = clamp(1 - (control - 50) / 160, 0.65, 1.3);
  const magBase = zone === 'good' ? 3 + Math.random() * 4 : 9 + Math.random() * 7;
  const mag = (magBase + gradeExtra) * ctlScale;
  const angle = Math.random() * Math.PI * 2;
  return { dx: Math.cos(angle) * mag, dy: Math.sin(angle) * mag };
}

// ------------------------------------------------------------------
// 飛行路徑（給前端動畫重播用）：3D 縱深呈現——
// 球從投手出手點（畫面上方中央、遠處小球）出現，朝自己的進壘位置
// 逼近並逐漸放大（由淺至深），最後在進壘點閃紅定格＝判定瞬間。
// 位移比照真實球路：
//   直球　：直線進壘、飛行時間最短（幾乎一閃而逝）
...
// ------------------------------------------------------------------
// 出手區域對球質的修正（第 3 點改版）：
//   ★完美：落點準之外，速球展現優良球速（飛行顯示時間 ×0.85＝一閃而逝）、變化球位移 ×1.25（更兇）
//   ○不錯：球速次佳（×0.95）、位移次佳（×1.05）
//   △勉強：球速下滑（×1.10）、位移變平（×0.80）
//   ✕走鐘：維持超慢紅中小便球（eephus 路徑，另行處理）
// v11：全部擊球判斷時間 ×1.5（打擊太難）——飛行顯示時間與揮棒時機窗同步放大
export const BAT_TIME_SCALE = 1.5;

export const RELEASE_ZONE_FX = {
  perfect: { speed: 0.85, drift: 1.25 },
  good: { speed: 0.95, drift: 1.05 },
  poor: { speed: 1.1, drift: 0.8 },
  worst: { speed: 1, drift: 1 },
};

export function buildFlightPath({ targetX, targetY, actualX, actualY, pitchTypeId, grade = 'B', effStuff = 50, eephus = false, releaseZone = 'good' }) {
  const gp = GRADE_PARAMS[grade] || GRADE_PARAMS.B;
  const fx = RELEASE_ZONE_FX[releaseZone] || RELEASE_ZONE_FX.good;
  const drift = gp.drift * fx.drift; // 位移＝球種等級 × 出手品質
  const spd = (ms) => Math.round(ms * fx.speed * BAT_TIME_SCALE); // 球速＝clamp 後再乘出手品質與全域放慢倍率
  // 出手點：好球帶上方中央（畫面內、遠處）——直球看起來就是「出現在該點、由遠而近」
  const startX = 50;
  const startY = 16;
  const st = effStuff - 50;
  const base = { startX, startY, endX: actualX, endY: actualY, pitchTypeId };

  // 失投小便球：整顆軟掉、超慢的高拋物線飄進紅中——打者有大把時間判定
  if (eephus) {
    return {
      ...base,
      motion: 'arc',
      controlX: (startX + actualX) / 2,
      controlY: (startY + actualY) / 2 - 18,
      durationMs: Math.round(1250 * BAT_TIME_SCALE),
      eephus: true,
    };
  }
  ...
  switch (pitchTypeId) {
    case 'slider': {
      ...
      return {
        ...
        tunnelY: actualY - 2,
        breakT: 0.52,
        durationMs: spd(clamp(560 - st * 2, 480, 650)),
      };
    }
    case 'curve': {
      ...
      return {
        ...
        motion: 'arc',
        controlX: (startX + actualX) / 2,
        controlY: (startY + actualY) / 2 - hump,
        durationMs: spd(clamp(790 - st * 2.5, 690, 890)),
      };
    }
    case 'fork': {
      ...
      return {
        ...
        breakT: 0.64,
        durationMs: spd(clamp(480 - st * 1.8, 400, 560)),
      };
    }
    case 'change': {
      ...
      return {
        ...
        tunnelY: clamp(actualY - drop, FIELD.canvasMin, FIELD.canvasMax),
        breakT: 0.7,
        durationMs: spd(clamp(550 - st * 2, 470, 630)),
      };
    }
    default: {
      ...
      return {
        ...base,
        motion: 'straight',
        durationMs: spd(clamp(400 - st * 1.5, 320, 470)),
        isFastball: true,
      };
    }
  }
...
export function swingWindowsOf(flightMs, mode = 'normal') {
  ...
  let perfect = clamp(dur * 0.12, 42 * BAT_TIME_SCALE, 85 * BAT_TIME_SCALE);
...
// ------------------------------------------------------------------
// 新版擊球判定：時機分 t × 位置分 p → 非線性合成總分 S → 錨點機率表內插
//   S = 100 × ((t+p)/200)^0.8 + 12 × (t/100) × (p/100)（雙高加乘），上限 100
// 錨點（一般打擊；表內沒有 K——「有碰到球」就不可能揮空，K 只留給真揮空）
// ------------------------------------------------------------------
export function compositeScore(t, p) {
  return clamp(100 * Math.pow((t + p) / 200, 0.8) + 12 * (t / 100) * (p / 100), 0, 100);
}

const S_ANCHORS = [
  [0, { SINGLE: 8, XBH: 2, HR: 0.5, OUT: 58, FOUL: 31.5 }],
  [15, { SINGLE: 10, XBH: 3, HR: 1, OUT: 56, FOUL: 30 }],
  [30, { SINGLE: 18, XBH: 6, HR: 2, OUT: 48, FOUL: 26 }],
  [50, { SINGLE: 30, XBH: 10, HR: 4, OUT: 38, FOUL: 18 }],
  [70, { SINGLE: 38, XBH: 15, HR: 7, OUT: 27, FOUL: 13 }],
  [90, { SINGLE: 42, XBH: 20, HR: 12, OUT: 16, FOUL: 10 }],
  [100, { SINGLE: 40, XBH: 24, HR: 18, OUT: 10, FOUL: 8 }],
];

// 依 S 於錨點間線性內插；強力打擊把一安份額搬去長打/全壘打（高風險高報酬）
export function tableAtScore(S, mode = 'normal') {
  let lo = S_ANCHORS[0];
  let hi = S_ANCHORS[S_ANCHORS.length - 1];
  for (let i = 0; i < S_ANCHORS.length - 1; i++) {
    if (S >= S_ANCHORS[i][0] && S <= S_ANCHORS[i + 1][0]) {
      lo = S_ANCHORS[i];
      hi = S_ANCHORS[i + 1];
      break;
    }
  ...
  const f = (S - lo[0]) / span;
  const w = {};
  for (const k of Object.keys(lo[1])) w[k] = lo[1][k] + (hi[1][k] - lo[1][k]) * f;
  if (mode === 'power') {
    w.SINGLE *= 0.5;
    w.XBH *= 1.3;
    w.HR *= 1.9;
...
export function resolvePitch({
  ...
  cor = 1, // 彈力係數（開房設定 0.5~1.5）：一安 ×c、長打 ×c^1.5、全壘打 ×c²
  batterStats,
  strikes = 0,
}) {
  ...
  if (batterMode === 'take') {
    ...
    if (batX == null || batY == null) {
      ...
      if (timingScore == null) return missResult; // 真揮空①：時機完全沒對上

      // 沒放開＝沒出棒：交給主審判（帶內好球、帶外壞球）
      if (batX == null || batY == null) {
        return {
          ...missResult,
          kind: withinZone ? 'CALLED_STRIKE' : 'BALL',
      ...
      const powerMode = batterMode === 'power';
      const sweetR = powerMode ? POWER_SWEET_RADIUS : SWEET_RADIUS;
      const contactR = powerMode ? POWER_CONTACT_RADIUS : CONTACT_RADIUS;
      const dx = batX - actualX;
      const dy = batY - actualY;
      const dist = Math.hypot(dx, dy);
      if (dist > contactR) return missResult; // 真揮空②：位置完全沒對上

      // ── 有碰到球：從這裡開始不可能是揮空 ──
      // 位置分 p：甜蜜圈內 85~100，接觸圈內線性 85→0（與時機分同形）
      const p =
        dist <= sweetR
          ? 100 - (dist / sweetR) * 15
          : 85 * (1 - (dist - sweetR) / (contactR - sweetR));

      // 非線性合成總分 S（雙高加乘）
      let S = compositeScore(timingScore, p);
      if (!withinZone) S *= 0.75; // 追打壞球硬碰：擊球品質折損
      if (hung) S = Math.min(100, S + 10); // 失投球（含小便球）就是好打
      S = clamp(S, 0, 100);

      // 切球程度：正＝削到球的上緣（→ 滾地）、負＝挖到下緣（→ 飛球）
      const cut = clamp(-dy / contactR, -1, 1);
      groundBias = cut;

      let w = tableAtScore(S, batterMode);

      // 深切修正：型態由擊球點決定——削頂太深長打下修成強勁滾地、挖底太深變高飛
      if (cut > 0.5) {
        w.HR *= 0.3;
        w.XBH *= 0.7;
        w.SINGLE *= 1.15;
      ...
      // 彈力係數（開房設定）：越長程受彈性影響越大
      const c = clamp(cor || 1, 0.5, 1.5);
      w.SINGLE *= c;
      w.XBH *= Math.pow(c, 1.5);
      w.HR *= c * c;

      // 既有修正鏈：打者/投手數值 → 球種等級 → 拿手球嘲諷 → 佈陣
      w = applyStatAdjustments(w, { ...batterEff, stuff: pitcherStats.stuff });
      const hf = gp.hitFactor;
      for (const key of ['HR', 'XBH', 'SINGLE']) {
        if (w[key] != null) w[key] = Math.max(0, w[key] * hf);
      }
      if ((grade === 'S' || grade === 'A') && S >= 50) {
        w = applyFavNerf(w);
      }
      w = applyShift(w, shift, batterMode);
      kind = weightedPick(w);
      contactQuality = Math.round(S);
    }
  }
...
const SWING_K_NARRATIONS = [
  ['投手出手——', '打者大棒一揮……揮空！', '三振出局！人體電風扇全速運轉，捕手興奮握拳'],
  ['決勝球出手——', '棒子掃過去，只切到一陣空氣！', '揮空三振！這風速，連觀眾席都感覺到涼意'],
  ['投手全力一投——', '打者奮力跟上……完全揮空！', '三振出局！大棒一揮掃出一陣強風——標準的人體電風扇！'],
  ['引誘球出手——', '打者忍不住出棒……棒子落空！', '揮空三振！投手握拳怒吼，打者懊惱地敲了敲頭盔'],
];
const LOOK_K_NARRATIONS = [
  ['這球直取好球帶邊角，打者棒子扛在肩上——', '主審毫不猶豫，右手用力一揮！', '站著不動遭到三振！打者搖著頭走回休息區'],
  ['決勝球切進來，打者完全沒有出棒的意思——', '主審大手一拉弓！', '冷凍的披薩——站著不動三振出局！'],
  ['球從邊角竄進好球帶，打者只能目送——', '主審高喊：好球三！', '眼睜睜看著好球進壘，站著吃下這個老 K！'],
  ['這顆進壘角度太刁鑽，打者判斷是壞球——', '主審不這麼認為，右手果斷一揮！', '站三振！打者不可置信地回頭看了主審一眼'],
];
...
export function applyOutcome(situation, kind, batterName, opts = {}) {
  ...
  switch (kind) {
    ...
    case 'K': {
      ...
      if (s.strikes >= 3) {
        s.outs += 1;
        s.log = kind === 'K' ? pickOne(SWING_K_LOGS) : pickOne(LOOK_K_LOGS);
        s.narration = kind === 'K' ? pickOne(SWING_K_NARRATIONS) : pickOne(LOOK_K_NARRATIONS);
        s.balls = 0;
        s.strikes = 0;
        s.strikeout = true;
        s.paEnded = true;
    ...
    case 'OUT': {
      ...
      // groundBias：來自打者揮擊時球棒相對球的上下位置（切到上緣＝滾地、下緣＝飛球）
      // 新制：擊球點強力決定型態——切得越偏，滾/飛幾乎是必然
      const groundBias = opts.groundBias || 0;
      const grounderProb = clamp(0.5 + groundBias * 0.55, 0.03, 0.97);
      ...
      // 犧牲飛球/滾地：三壘跑者回本壘
      // 犧牲得分（三壘有人＋原本未滿兩人出局＋界內球）：直接算犧牲得分
      // ——只要不是這一球製造的第三個出局，滾/飛都算犧牲得分
      const forceScore = s.bases.third && s.outs < 3 && situation.outs < 2;
      if (forceScore) {
        s.runsThisPlay += 1;
        s.bases.third = null;
        s.log += grounder ? '（三壘跑者犧牲滾地得分）' : '（三壘跑者犧牲高飛得分）';
        s.narration.push(grounder
          ? '三壘跑者早就起跑——趁著滾地球回本壘得分！'
          : '三壘跑者離壘等球——接殺瞬間踏壘起跑，穩穩踩上本壘板得分！');
      } else if (s.bases.third && s.outs < 3) {
        let sacChance = 0.35;
