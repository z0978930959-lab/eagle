...
// ------------------------------------------------------------------
// 飛行路徑（給前端動畫重播用）：二次貝茲曲線規格。直球幾乎沒有弧線、
// 飛行時間極短（近乎一瞬間）；變化球依球種位移方向偏移控制點，等級
// 越高（S/A 拿手）偏移越大、飛行時間隨球威縮短（更急更快）。
// ------------------------------------------------------------------
export function buildFlightPath({ targetX, targetY, actualX, actualY, pitchTypeId, grade = 'B', effStuff = 50 }) {
  const pitchType = PITCH_TYPE_MAP[pitchTypeId];
  const gp = GRADE_PARAMS[grade] || GRADE_PARAMS.B;
  const isFastball = !pitchType.breakDir;
  const startX = 50;
  const startY = -12;
  ...
  if (isFastball) {
    durationMs = Math.round(clamp(320 - (effStuff - 50) * 1.2, 260, 380));
    controlX = (startX + actualX) / 2;
    controlY = (startY + actualY) / 2;
  } else {
    durationMs = Math.round(clamp(950 - (effStuff - 50) * 4, 700, 1150));
    const bendMag = 22 * gp.drift;
    controlX = (startX + actualX) / 2 + (pitchType.breakDir === 'outer' ? bendMag : 0);
    // low 系：控制點往上偏移（y 變小），營造「先浮高、後墜落」的視覺
    controlY = (startY + actualY) / 2 + (pitchType.breakDir === 'low' ? -bendMag * 0.6 : 0);
  }

  return { startX, startY, controlX, controlY, endX: actualX, endY: actualY, durationMs, isFastball };
}

// 揮擊命中分級（連續座標版）：球棒最終位置 vs 實際進壘點的距離
//  exact 甜蜜點全中 / near 有效接觸範圍內 / far 完全揮偏 / chase 追打壞球區（不論多準都是最差級）
export const SWEET_RADIUS = 9;
export const CONTACT_RADIUS = 22;
export const POWER_SWEET_RADIUS = 5.5;
export const POWER_CONTACT_RADIUS = 15;
...
export function resolvePitch({
  ...
  if (batterMode === 'take') {
    ...
    // 觸擊：用球棒-球距離衍生的 contactQuality 取代舊的時機條分數
    const geo = classifySwingGeo({ actualX, actualY, batX, batY, powerMode: false });
    contactQuality = geo.contactQuality;
    if (!withinZone) {
      // 壞球：依選球眼決定是否收棒
