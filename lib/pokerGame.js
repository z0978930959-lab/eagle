 ...
 * 破產出局、8 回合、真正平手（下注各自拿回）。
 *
 * 一場：雙方各 100 枚，8 回合。每回合流程：
 *   1. 各下底注 5（不足 5 → 出局）
 *   2. 各發 5 張
 ...
 *   7. 無人棄牌 → 開牌比大小
 *
 * ★ 破限加注（Show Hand）：每人「每一場」有一次機會，把加注目標拉到
 *   50 為止，無視該階段的 10／25 上限。用掉就沒了，下一場才會重置。
 *
 * 換牌先手：8 回合 A/B 交替（回合 1 A 先、回合 2 B 先…）
 * ------------------------------------------------------------------ */
...
const BET1_MAX = 10;
const BET2_MAX = 25;
const SUPER_MAX = 50; // 破限加注上限（每人每場一次）
...
function startGame(room) {
  ...
  room.pk = {
    ...
    superUsed: { a: false, b: false }, // 破限加注（上限 50）是否已用掉——每場重置
...
function startRound(room) {
  ...
  if (pk.round >= TOTAL_ROUNDS) {
    ...
    pushLog(pk, `🏁 ${TOTAL_ROUNDS} 回合結束（A ${pk.chips.a} / B ${pk.chips.b}）`);
    settleGame(room);
    return;
  }
  ...
  const firstDrawer = pk.round % 2 === 1 ? 'a' : 'b'; // 奇數回合 A 先、偶數回合 B 先
...
export function actPokerBet(room, role, payload) {
  ...
  if (move === 'bet' || move === 'raise') {
    // amount = 「加到多少」的目標注額（當前注額），不是增量。
    // 例：我下 2（level=2）→ 對方加到 5（level=5）→ 我加到 6~10…
    // 任一方加到上限（bet1=10 / bet2=25），對方只能跟注或棄牌。
    const target = Number(payload?.amount);
    if (!Number.isInteger(target)) throw new Error('BAD_INPUT');
    const curLevel = Math.max(r.bets.a, r.bets.b); // 本回合當前注額
    if (target <= curLevel) throw new Error('BAD_INPUT'); // 加注必須高於當前注額

    // ★ 破限加注（Show Hand）：每人每場一次，可把目標拉到 SUPER_MAX。
    //   只有「目標真的超過該階段上限」時才算用掉機會——玩家不小心勾了破限
    //   但只加到上限以內，就當一般加注處理，不白白浪費那一次。
    pk.superUsed = pk.superUsed || { a: false, b: false };
    const useSuper = payload?.super === true && target > cap;
    if (useSuper && pk.superUsed[role]) throw new Error('SUPER_USED');
    const effCap = useSuper ? SUPER_MAX : cap;
    if (target > effCap) throw new Error('OVER_CAP'); // 不得超過（可能已放寬的）上限

    const pay = target - r.bets[role]; // 只需補到目標注額的差額
    if (pay > pk.chips[role]) throw new Error('NOT_ENOUGH_CHIPS');
    if (useSuper) {
      pk.superUsed[role] = true;
      pushLog(pk, `💥 ${seatName(role)} 使用破限加注——直接加到 ${target}（機會已用完）`);
    }
    pk.chips[role] -= pay;
    r.bets[role] += pay;
    r.pot += pay;
...
export function actPokerDraw(room, role, payload) {
  ...
  // 換牌只換掉選定的位置，其餘牌維持原位（不重排整手）
  const fresh = r.deck.splice(0, actualDiscards.length);
  let fi = 0;
  r.hands[role] = hand.map((card, i) => (actualDiscards.includes(i) ? fresh[fi++] : card));
...
function doShowdown(room) {
  ...
  if (cmp === 0) {
    ...
    r.roundWinner = null; // 平手無金邊
    pushLog(pk, `🤝 平手（雙方 ${sa.name}）——下注各自拿回`);
  } else {
    const winner = cmp > 0 ? 'a' : 'b';
    pk.chips[winner] += r.pot;
    r.roundWinner = winner; // 記錄勝方，前端亮金邊
    pushLog(pk, `🎴 開牌：A ${sa.name} vs B ${sb.name} → ${seatName(winner)} 勝，收 ${r.pot}`);
  }
  pk.seq += 1;
  r.stage = 'showdown'; // 停在開牌狀態，等雙方看完動畫再推進
  r.nextReady = { a: false, b: false };
}

// 開牌後推進下一回合（前端播完翻牌＋金邊動畫自動送出；雙方都送才進下一回合）
// 註：此動作為 idempotent——前端會重試，若已不在開牌狀態（對方先推進、或本場已結束）
// 就直接當作已完成、安靜返回，不可拋錯，否則重試會被當成失敗。
export function actPokerNextRound(room, role) {
  const pk = room.pk;
  if (!pk) throw new Error('NOT_STARTED');
  if (pk.phase !== 'round' || pk.r?.stage !== 'showdown') return;
  pk.r.nextReady = pk.r.nextReady || { a: false, b: false };
  pk.r.nextReady[role] = true;
  if (pk.r.nextReady.a && pk.r.nextReady.b) {
    startRound(room);
  }
}
...
const PK_STICKERS = [
  ...
  'taunt_6', 'taunt_7', 'taunt_8', 'taunt_9', 'taunt_10',
];

export function actPokerSticker(room, role, payload) {
  const pk = room.pk;
  if (!pk) throw new Error('NOT_STARTED');
  const name = payload?.name;
  if (!PK_STICKERS.includes(name)) throw new Error('BAD_INPUT');
  // 不設任何次數或間隔限制：雙方可同時按，也可以連發。
  // 狀態掛在 room 上（見 lib/stickers.js）：BO3 換場會重建 room.pk，
...
export function pokerViewFor(room, role) {
  ...
  base.stickers = stickerView(room);
  ...
  if (r) {
    view.round_state = {
      ...
      // 破限加注（每人每場一次，上限 50）：剩餘機會與上限，給前端畫按鈕用
      superMax: SUPER_MAX,
      superLeft: { me: !pk.superUsed?.[role], opp: !pk.superUsed?.[opp] },
      deckLeft: r.deck.length,
      // 自己的手牌永遠可見；對手手牌只有開牌後可見
      myHand: r.hands[role],
      // 目前手牌牌型（讓玩家知道自己現在是什麼牌）
      myHandType: r.hands[role].length === 5 ? evaluateHand(r.hands[role]).name : null,
      ...
      reveal: r.reveal
        ...
        : null,
      // 開牌後：勝方（'a'/'b'/null 平手）→ 前端在勝方牌型亮金邊；iWonRound 方便前端判斷
      roundWinner: r.roundWinner ?? null,
      iWonRound: r.roundWinner ? r.roundWinner === role : null,
      // 只有「對局進行中」的開牌才 hold；本場已結束（gameover）時 pk.r 仍停在
      // showdown，若不檢查 phase，前端會在結算畫面上不斷嘗試推進下一回合。
      showdownHold: pk.phase === 'round' && r.stage === 'showdown',
    };
  }
  return view;
}
