...
export function actPokerBet(room, role, payload) {
  ...
  if (move === 'bet' || move === 'raise') {
    ...
    if (target > cap) throw new Error('OVER_CAP'); // 不得超過該階段上限
...
const PK_STICKERS = ['taunt_1', 'taunt_2', 'taunt_3', 'taunt_4', 'taunt_5', 'taunt_6'];
...
export function actPokerSticker(room, role, payload) {
  ...
  const now = Date.now();
  pk.stickerAt = pk.stickerAt || {};
  if (now - (pk.stickerAt[role] || 0) < 2000) throw new Error('STICKER_LIMIT'); // 2 秒防連點
  pk.stickerAt[role] = now;
  pk.lastSticker = { role, name, ts: now, seq: (pk.seq += 1) };
}
...
export function pokerViewFor(room, role) {
  ...
  base.lastSticker = pk.lastSticker || null;
