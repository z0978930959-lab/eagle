...
export function actPokerBet(room, role, payload) {
  ...
  if (move === 'bet' || move === 'raise') {
    ...
    // 任一方加到上限（bet1=10 / bet2=15），對方只能跟注或棄牌。
...
export function actPokerNextRound(room, role) {
  const pk = room.pk;
  if (!pk || pk.phase !== 'round' || pk.r.stage !== 'showdown') throw new Error('WRONG_PHASE');
...
const PK_STICKERS = ['taunt_1', 'taunt_2', 'taunt_3'];
