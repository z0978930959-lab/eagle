...
export function actPokerBet(room, role, payload) {
  ...
  if (move === 'bet' || move === 'raise') {
    ...
    // 本階段「加注量」上限 = cap，且不超過剩餘籌碼(all-in)
    const raiseRoom = Math.min(cap, pk.chips[role] - Math.max(0, toCall)) ;
    if (amount > raiseRoom + 0) {
      // 允許 all-in 低於 cap 的情形：若 amount 恰為剩餘全部則放行
      if (Math.max(0, toCall) + amount !== pk.chips[role]) throw new Error('OVER_CAP');
    }
