...
export function actMissionClue(room, seat, payload) {
  ...
  ms.clue = { word, count, remaining: count + 1 };
  ms.phase = 'guess';
...
const MS_STICKERS = ['taunt_1', 'taunt_2', 'taunt_3'];
...
export function actMissionSticker(room, seat, payload) {
  ...
  const now = Date.now();
  ms.stickerAt = ms.stickerAt || {};
  if (now - (ms.stickerAt[seat] || 0) < 2000) throw new Error('STICKER_LIMIT');
  ms.stickerAt[seat] = now;
  ms.lastSticker = { seat, name, ts: now, seq: (ms.seq += 1) };
}
...
export function missionViewFor(room, seat) {
  ...
  return {
    ...
    lastSticker: ms.lastSticker || null,
