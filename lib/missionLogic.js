...
// v36：貼圖擴充到 8 張（前端 components/Mission.jsx 的清單要一致）
...
export function missionViewFor(room, seat) {
  ...
  return {
    ...
    // 終局（勝利／逾時／踩炸彈）後把整個盤面攤開給雙方看：
    // 猜的人這時也該看得到剩下的關鍵人物與炸彈藏在哪，才有「原來如此」的收尾。
    // revealed 仍然只代表「這局真的被翻開過」，方便前端把「翻過的」與「事後公開的」畫得不一樣。
    cells: ms.cells.map((c) => ({
      word: c.word,
      revealed: c.revealed,
      role: c.revealed ? c.role : null,
      img: c.revealed ? c.img : null,
      answer: !c.revealed && (isGuide || isOver) ? c.role : null,
    })),
    log: ms.log,
    seq: ms.seq,
  };
