...
export function generateBoard() {
  ...
  // 路線一：炸彈只能壓在「雙方都是路人」的格子（rest），
  // 絕不壓在任何一方的關鍵格 —— 這樣 15 個關鍵永遠可安全翻出，每局必定可解。
  // A 與 B 的炸彈可落在同一格（雙方炸彈），也可各自獨立；一律取自 rest。
  const aBombPool = shuffle(rest.slice());
  for (const i of aBombPool.slice(0, BOMBS_EACH)) aRole[i] = 'bomb';

  const bBombPool = shuffle(rest.slice());
...
export function actMissionGuess(room, role, payload) {
  ...
  // 翻牌依「雙方身分」的優先級決定結果：炸彈 > 關鍵 > 路人
  // 只要任一方是炸彈就爆；沒炸彈但任一方是關鍵就得分；雙方都路人才是路人。
  let shown;
  if (cell.aRole === 'bomb' || cell.bRole === 'bomb') shown = 'bomb';
  else if (cell.aRole === 'key' || cell.bRole === 'key') shown = 'key';
  else shown = 'civ';

  cell.revealed = true;
  cell.revealedBy = guesser;
  cell.shownRole = shown;
  // 從該身分的圖池取出一張（不放回）；池空了才循環，保證盤面上不會有兩張相同
  const pool = ms.imgPool[shown];
  cell.img = pool.length ? pool.shift() : 1 + randInt(shown === 'key' ? KEY_IMG_COUNT : shown === 'civ' ? CIV_IMG_COUNT : BOMB_IMG_COUNT);
  ms.seq += 1;
  ms.lastReveal = { index: i, role: shown, seq: ms.seq };
