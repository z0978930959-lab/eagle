...
const MS_STICKERS = ['taunt_1', 'taunt_2', 'taunt_3', 'taunt_4', 'taunt_5', 'taunt_6', 'taunt_7', 'taunt_8'];
...
export function missionViewFor(room, seat) {
  ...
  // v36：終局（不論成功／踩炸彈／回合用完）一律對「雙方」攤牌——
  // 沒翻到的格子也把身分與圖片一起送出，猜的人才看得到炸彈藏在哪、還有哪些關鍵沒找到。
  const finalReveal = ms.phase === 'over';
  ...
  return {
    ...
    cells: ms.cells.map((c) => ({
      word: c.word,
      revealed: c.revealed,
      // 終局時連沒翻開的格子也一併公開身分與圖片（前端會用暗色標示「未翻開」）
      role: c.revealed || finalReveal ? c.role : null,
      img: c.revealed || finalReveal ? c.img : null,
      answer: !c.revealed && (isGuide || finalReveal) ? c.role : null,
    })),
    log: ms.log,
    seq: ms.seq,
  };
