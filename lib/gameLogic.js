...
/* ------------ 彩蛋：投降輸一半 ------------ */
// 觸發方式（前端）：連點三下「直球」或「打擊」按鈕。發起後對方大屏幕會看到
// 經典畫面並選擇是否接受；接受＝發起方認輸、比賽立即結束（這裡流行投降輸一半）。

export function actSurrenderOffer(room, role) {
  const g = room.game;
  ...
  g.surrender = { by: role, status: 'pending' };
  pushLog(g, `🏳️ ${g[role].team.short} 舉起白旗，向對方提出「投降輸一半」！全場譁然——`);
}
...
export function actSurrenderRespond(room, role, payload) {
  ...
  if (payload?.accept) {
    ...
    g.surrender = null;
    pushLog(g, `🏳️ ${acceptSide.team.short} 把白旗丟了回去：「想投降？把比賽打完！」比賽繼續！`);
  }
}
