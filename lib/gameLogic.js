...
export function joinRoom(room, homeTeamId) {
  ...
  room.game = {
    ...
    // 挑釁系統：stage 1~4 對應四張梗圖；每次挑釁 40% 觸發裁判警告，累計兩次總教練驅逐
    taunts: { away: { stage: 0, warnings: 0, ejected: false }, home: { stage: 0, warnings: 0, ejected: false } },
    tauntSeq: 0,
...
function commitPitch(room, { typeId, targetX, targetY, waste, shift, release, auto }) {
  ...
  const releaseZone = auto ? 'worst' : releaseZoneOf(release);
  ...
  const path = buildFlightPath({ targetX, targetY, actualX, actualY, pitchTypeId: typeId, grade, effStuff: pitcher.effStuff, eephus });
...
/* ------------ 挑釁系統 ------------ */
// 任何時候都能按（終局除外）。第 1~4 次各對應一張梗圖（之後重複第 4 張），
// 對方大屏幕會播出。每次挑釁 40% 機率觸發主審警告；同一方累計兩次警告，
// 總教練被驅逐——之後無法下達「盜壘、代打、佈陣」戰術（換投、牽制不受影響）。
const TAUNT_LINES = [
  '「回家吧！下半場沒得踢啦！」',
  '「要不要一會我下去跳舞撐撐場面？」',
  '「跳舞也輪不到你跳，我跳！」',
...
export function actTaunt(room, role) {
  ...
  me.stage = Math.min(me.stage + 1, 4);
  const short = g[role].team.short;
  pushLog(g, `😤 ${short} 對著對面板凳喊話：${TAUNT_LINES[me.stage - 1]}`);

  let warned = false;
  let justEjected = false;
  if (!me.ejected && Math.random() < 0.4) {
    warned = true;
    me.warnings += 1;
    if (me.warnings >= 2) {
      me.ejected = true;
      ...
      pushLog(g, `🟨 主審臉色一沉，對 ${short} 板凳提出警告！（累計 ${me.warnings} 次，兩次驅逐總教練）`);
    }
  }
  g.tauntSeq += 1;
  g.tauntFeed = { seq: g.tauntSeq, by: role, stage: me.stage, warned, ejected: justEjected };
}
