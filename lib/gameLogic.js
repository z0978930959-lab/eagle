...
function commitPitch(room, { typeId, targetX, targetY, waste, shift, release, auto }) {
  ...
  const releaseZone = auto ? 'worst' : releaseZoneOf(release, pitcher.stamina);
...
/* ------------ 挑釁系統 ------------ */
// 任何時候都能按（終局除外）。第 1~11 次各對應一張梗圖（第 11 張＝摔倒，之後
...
export const TAUNT_WARN_CHANCE = 0.2; // 每次挑釁觸發主審警告的機率（單一判定點，保證真的是 20%）
...
export function actTaunt(room, role) {
  ...
  let forfeited = false;
  if (me.warnings < 4 && Math.random() < TAUNT_WARN_CHANCE) {
    warned = true;
    me.warnings += 1;
    if (me.warnings === 1) {
