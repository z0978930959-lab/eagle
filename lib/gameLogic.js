...
import { resolvePitch, applyOutcome, resolveSteal, applyStealResult, timingLabelOf } from './engine.js';
...
export function actPitcherSubmit(room, role, payload) {
  ...
  let { typeId, zoneTarget, shift = 'normal' } = payload || {};
  // 防禦性正規化：ZoneGrid 元件多選模式會回傳陣列，若前端誤傳 ['1-1'] 這裡取第一格救回，
  // 避免合法操作被判 BAD_INPUT 卡到超時失投
...
function resolveBatterPlay(room, payload) {
  ...
  // 揮棒時機分數：白名單為 0~100 數字或 null；看球不揮棒一律視為 null
  ...
  if (stealTarget) {
    ...
    const success = resolveSteal({
      ...
      heldClose: g.heldClose || 0, // 本球前被牽制回壘的次數：離壘變小，起跑更難
    });
  ...
  g.lastResult = {
    ...
    batterChoice: { mode, guessCat: guessCat || null, guessSpecies: guessSpecies || null, guessZoneKind: guessZoneKind || 'cell', guessZone: guessZone || null, steal: steal || null, timing, timingLabel: timingLabelOf(timing) },
