...
import { resolvePitch, applyOutcome, resolveSteal, applyStealResult, applyDoubleStealResult, computeActualZone, gradeOf, timingLabelOf } from './engine.js';
...
export const PHASE_MS = 25000; // 投球/打擊選擇時限 25 秒
...
export function actPitcherSubmit(room, role, payload) {
  ...
  let { typeId, zoneTarget, shift = 'normal', release } = payload || {};
  // 防禦性正規化：若前端誤傳陣列取第一格救回，避免合法操作被判 BAD_INPUT 卡到超時失投
  if (Array.isArray(zoneTarget) && zoneTarget.length === 1) zoneTarget = zoneTarget[0];
  ...
  commitPitch(room, { typeId, zoneTarget, shift: shift || 'normal', release, auto: false });
}

// 投球落地流程（真人投球與超時自動失投共用）：
// 1. 出手時機差勁 → 直接失投紅中
// 2. 觸身球在此先判（瞄內角/壞球區、控球差、疲勞高機率上升），死球直接進結果
// 3. 依球種等級算實際落點，存進 pendingPitch 供打者端閃現
function commitPitch(room, { typeId, zoneTarget, shift, release, auto }) {
  const g = room.game;
  const fKey = fieldingKey(g.half);
  const bKey = battingKey(g.half);
  const fieldingSide = g[fKey];
  ...
  // 觸身球：瞄內角或壞球區時，控球越差機率越高
  if (!auto && !badRelease) {
    const aimingInside = zoneTarget === 'waste' || zoneTarget.split('-')[1] === '0';
    if (aimingInside) {
      const hbpChance = Math.min(0.06, Math.max(0.008, 0.035 - (effControl - 50) / 600));
      if (Math.random() < hbpChance) {
        resolveHbp(room, { typeId, zoneTarget, shift, batter, pitcher, battingSide, fieldingSide });
        return;
      }
    }
  }
  ...
  if (auto || badRelease) {
    ...
    actualZone = computeActualZone(zoneTarget, typeId, effControl, grade);
    hung = actualZone === '1-1' && zoneTarget !== '1-1'; // 非瞄準紅中卻跑進紅中＝失投
  }

  const pitchType = PITCH_TYPES.find((t) => t.id === typeId);
  // 打者反應窗（毫秒）：直球短而老實；變化球長但會「轉彎」騙人；球威越高越短
  const isBreaking = !!pitchType.breakDir;
  const baseWindow = isBreaking ? 950 : 700;
  const flashWindow = Math.max(480, Math.min(1050, baseWindow - (pitcher.effStuff - 50) * 3));
  ...
  g.pendingPitch = {
    ...
    hung,
    auto,
    flash: { zone: actualZone, breakDir: pitchType.breakDir, window: flashWindow },
  };
  g.phase = 'batter';
...
function resolveHbp(room, { typeId, zoneTarget, shift, batter, pitcher, battingSide, fieldingSide }) {
  const g = room.game;
  const prevBases = { ...g.bases };
  const prevSpeeds = { ...g.baseSpeeds };
  const situation = applyOutcome(
  ...
  g.lastResult = {
    ...
    hung: false,
    grade: null,
    pitcherChoice: { typeId, zoneTarget, shift },
...
function resolveBatterPlay(room, payload) {
  ...
  // 揮擊格：反應窗內點的格子（'r-c'｜'out' 揮向壞球區｜null 沒出棒）
  let swungCell = payload?.swungCell;
  if (swungCell != null && swungCell !== 'out' && !ZONE_IDS.has(swungCell)) throw new Error('BAD_INPUT');
  if (swungCell === undefined) swungCell = null;
  if (mode === 'take') swungCell = null;

  // 反應/時機分數
  let timing = payload?.timing;
  if (timing != null) {
    if (typeof timing !== 'number' || !Number.isFinite(timing)) throw new Error('BAD_INPUT');
    timing = Math.max(0, Math.min(100, Math.round(timing)));
  } else {
    timing = null;
  }
  if (mode === 'take' || swungCell == null) timing = mode === 'bunt' ? timing : null;
  ...
  const result = resolvePitch({
    actualZoneKey: g.pendingPitch.actualZone,
  ...
  g.prePlay = {
    ...
    opts: { shift, mode, runnerOnFirstSpeed, runnersGoing },
    hadSteal: false,
    challenged: false,
    closePlay: false,
  };
  ...
  const situation = applyOutcome(
    ...
    result.kind,
    batter.name,
    { shift, mode, runnerOnFirstSpeed, runnersGoing }
  );
  ...
  g.lastResult = {
    ...
    hung: result.hung,
    grade,
    pitcherChoice: { typeId: g.pendingPitch.typeId, zoneTarget: g.pendingPitch.zoneTarget, shift, grade, release: g.pendingPitch.release ?? null },
    batterChoice: { mode, swungCell, steal: steal ? steal.base : null, timing, timingLabel: timingLabelOf(timing) },
...
export function enforceTimeouts(room) {
  ...
  if (g.phase === 'pitcher') {
    ...
    commitPitch(room, { typeId: 'fastball', zoneTarget: '1-1', shift: 'normal', release: null, auto: true });
    return true;
  }
  if (g.phase === 'batter') {
    // 打者超時：自動不揮棒
...
export function viewFor(room, role) {
  ...
  const view = {
    ...
    game: {
      ...
      // 打者看得到佈陣與「閃現資訊」（實際進壘點＋位移方向＋反應窗，即時反應小遊戲用）；
      // 球種名稱與投手瞄準點仍然保密到結果揭曉
      pendingPitch: g.pendingPitch
        ? role === fieldingKey(g.half)
          ? { typeId: g.pendingPitch.typeId, zoneTarget: g.pendingPitch.zoneTarget, shift: g.pendingPitch.shift, grade: g.pendingPitch.grade }
          : { shift: g.pendingPitch.shift, flash: g.pendingPitch.flash }
        : null,
    },
  };
  return view;
