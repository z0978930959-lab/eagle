...
function commitPitch(room, { typeId, targetX, targetY, waste, shift, release, auto }) {
  ...
  if (eephus) {
    ...
    // [v36 修正] 失投判定只在「△勉強」時才成立。
    // 舊版對 ○不錯 也套同一條規則：不錯區本來就會有 3~7 的微幅偏移，
    // 只要原本瞄的是邊角、剛好往中間偏一點點，就被標成「失投」並送打者 +10 打擊分，
    // 造成玩家明明停在最好／次好區域卻被判失投的狀況。
    //   ★完美：零偏移，指哪投哪——永遠不可能失投
    //   ○不錯：微偏是設計內的正常結果——不算失投
    //   △勉強：大偏才有失投風險，且必須真的滑進紅中正中央（距中心 <5）
    //           而原本瞄的點確實遠離中心（>14）才算「本來要投邊角、結果送到紅中」
    hung =
      releaseZone === 'poor' &&
      Math.hypot(actualX - 50, actualY - 50) < 5 &&
      Math.hypot(targetX - 50, targetY - 50) > 14;
  }
