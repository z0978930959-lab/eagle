...
function freshTeamState(team) {
  return {
    ...
    score: 0,
    stats: freshStats(),
    mvp: {}, // 逐打者累計貢獻（賽後 MVP 圖卡用）：{ 姓名: { name, hits, hr, rbi, k } }
    forceRight: false, // [取消左打]：該玩家勾選後，全隊打擊一律以右打計算（含左右對決與球棒鏡像）
...
/* ---------------- 氣勢（Momentum） ----------------
 * g.momentum：單一有號數值 [-MOM_MAX, +MOM_MAX]，>0 倒向客隊(away)、<0 倒向主隊(home)。
 * 雙方大屏幕都看得到這條，是公開資訊。好結果推向該方，每球輕微衰減回中。
 * 氣勢高的一方在下一個動作獲得小幅加成（打者準度／投手控球，封頂 ±MOM_EDGE）。
 * 挑釁與氣勢綁定：原本氣勢在你這邊＝壓過對方；在對方那邊＝反噬——冒風險才有意義。 */
const MOM_MAX = 100;
const MOM_EDGE = 4; // 氣勢滿檔時對準度／控球的最大加成
const MOM_DECAY = 0.9; // 每球向中衰減

function clampMom(v) {
  return Math.max(-MOM_MAX, Math.min(MOM_MAX, Math.round(v)));
}
// side: 'away' | 'home'；amount>0 把氣勢推向該方
function addMomentum(g, side, amount) {
  const sign = side === 'away' ? 1 : -1;
  g.momentum = clampMom((g.momentum || 0) + sign * amount);
}
// 傳回該方目前的氣勢優勢值（>0 表示氣勢在這一邊）
function momentumFor(g, side) {
  return side === 'away' ? (g.momentum || 0) : -(g.momentum || 0);
}
// 該方目前可獲得的小幅能力加成（±MOM_EDGE）
function momentumEdge(g, side) {
  return Math.round((momentumFor(g, side) / MOM_MAX) * MOM_EDGE);
}

// 賽後 MVP：合併雙方逐打者累計，取貢獻分最高者（平手時偏向勝方）
function computeMvp(g) {
  const pick = (sideKey) => {
    const map = g[sideKey]?.mvp || {};
    const side = g[sideKey];
    let best = null;
    for (const p of Object.values(map)) {
      const score = p.hits + p.hr * 3 + p.rbi * 2;
      if (score <= 0) continue;
    ...
    return {
      name: best.name,
      teamShort: side.team.short,
      teamColor: side.team.color,
      hits: best.hits,
      hr: best.hr,
      rbi: best.rbi,
      score: best.score,
  ...
  if (h && !a) return h;
  // 兩邊都有：分數高者優先；同分偏向勝方
  if (a.score !== h.score) return a.score > h.score ? a : h;
  const winnerName = g.winner;
  if (winnerName === g.away.team.name) return a;
  if (winnerName === g.home.team.name) return h;
  return a;
}
...
function applyBoxStats({ battingSide, fieldingSide, kind, situation, batterName }) {
  ...
  // 逐打者 MVP 累計（賽後圖卡用）
  const mvpMap = battingSide.mvp || (battingSide.mvp = {});
  const mv = mvpMap[batterName] || (mvpMap[batterName] = { name: batterName, hits: 0, hr: 0, rbi: 0, k: 0 });
  mv.rbi += runs;
  if (situation.strikeout) mv.k += 1;
  if (kind === 'SINGLE' || kind === 'BUNT_HIT' || kind === 'XBH') mv.hits += 1;
  if (kind === 'HR') {
    mv.hits += 1;
    mv.hr += 1;
  }
...
// 依房間設定建立一場全新比賽（joinRoom 首次開局與 actRematch 重新開局共用）
function buildGame(room) {
  const awayTeam = TEAMS.find((t) => t.id === room.awayTeamId);
  const homeTeam = TEAMS.find((t) => t.id === room.homeTeamId);
  ...
  return {
    ...
    surrender: null, // 彩蛋：投降輸一半（{ by: 'away'|'home', status: 'pending'|'accepted', kind }）
    momentum: 0, // 氣勢條（公開）：>0 倒向客隊、<0 倒向主隊，範圍 ±100
    cor: room.cor || 1, // 彈力係數（開房設定）
    // 挑釁系統：stage 1~11 對應梗圖（>11 停留在摔倒）；洗牌袋制每 5 次挑釁保證恰好 1 次警告
    // 警告階梯（v11 五階）：1＝純警告；2＝總教練出場（禁代打）；3＝副教練出場（禁換投、代打）；
    // 4＝教練團全數出場（禁換投、代打、盜壘、牽制＋送對方一分）；5＝裁定比賽、對方勝利
    taunts: {
      away: { stage: 0, warnings: 0, ejected: false, coachEjected: false, allOut: false },
      home: { stage: 0, warnings: 0, ejected: false, coachEjected: false, allOut: false },
    },
    tauntSeq: 0,
...
// [取消左打]：勾選方全隊打擊一律以右打計算（含左右對決與球棒鏡像）。可隨時切換。
export function actForceRight(room, role, payload) {
  const g = room.game;
  if (!g) throw new Error('WRONG_PHASE');
  g[role].forceRight = !!(payload?.on ?? payload?.value ?? payload);
}

// 重新開局（投降平手後、或比賽結束後）：沿用同房號、同兩隊、同設定重開一場
export function actRematch(room, role) {
  const g = room.game;
  if (!g || g.phase !== 'gameover') throw new Error('WRONG_PHASE');
  room.game = buildGame(room);
  room.status = 'playing';
  return room;
}
...
export function actDeclareSqueeze(room, role, payload) {
  const g = room.game;
  if ((g.taunts?.[role]?.warnings || 0) >= 4) throw new Error('EJECTED'); // 教練團全數驅逐：不能下強迫取分
...
export function actDeclareSteal(room, role, payload) {
  const g = room.game;
  if ((g.taunts?.[role]?.warnings || 0) >= 4) throw new Error('EJECTED'); // 教練團全數驅逐：不能下盜壘暗號
...
export function actChangePitcher(room, role, idx) {
  const g = room.game;
  if ((g.taunts?.[role]?.warnings || 0) >= 3) throw new Error('EJECTED'); // 副教練出場：不能換投
  if (g.phase !== 'pitcher') throw new Error('WRONG_PHASE');
  if (role !== fieldingKey(g.half)) throw new Error('NOT_YOUR_TURN');
  const side = g[role];
...
export function actPinchHit(room, role, benchIdx) {
  const g = room.game;
  if ((g.taunts?.[role]?.warnings || 0) >= 2) throw new Error('EJECTED'); // 總教練出場：不能代打
...
export function actPitcherSubmit(room, role, payload) {
  ...
  if (payload.pitchOut) {
    const poShift = SHIFT_IDS.has(payload.shift) ? payload.shift : 'normal';
...
function commitPitch(room, { typeId, targetX, targetY, waste, shift, release, auto }) {
  ...
  if (eephus) {
    ...
    // 失投判定（v36 修正）：
    // 舊版只看「實際落點是否落在紅中方框內、且瞄準點離中心 >10」，兩個門檻靠太近——
    // ★完美／○不錯這種本來就該精準的出手，只要瞄準點稍微離開中心一點點（例如 11），
    // 再加上不錯區本來就有的微幅偏移，就會被誤判成失投，這是不合理的。
    // 新規則：
    //   ★完美：指哪投哪，永遠不算失投
    //   ○不錯：只是微偏，一樣不算失投
    //   △勉強：才有可能失投——而且必須是「本來瞄得遠遠的邊角／壞球區，
    //           結果整顆球飄進正中央」才算（用半徑而不是方框，避免邊界怪異）
    if (releaseZone === 'poor') {
      const aimDist = Math.hypot(targetX - 50, targetY - 50);
      const actDist = Math.hypot(actualX - 50, actualY - 50);
      hung = aimDist > 16 && actDist < 6;
    } else {
      hung = false;
    }
  }
...
export function actPickoff(room, role) {
  const g = room.game;
  if ((g.taunts?.[role]?.warnings || 0) >= 4) throw new Error('EJECTED'); // 教練團全數驅逐：不能牽制
  if (g.phase !== 'pitcher') throw new Error('WRONG_PHASE');
  if (role !== fieldingKey(g.half)) throw new Error('NOT_YOUR_TURN');
...
function resolveBatterPlay(room, payload) {
  ...
  // [取消左打]：勾選方全隊打擊以右打計算（左右對決一併改為右打）
  const effBats = battingSide.forceRight ? 'R' : batter.bats;
  // 氣勢加成（±MOM_EDGE）：氣勢在打者這邊→準度微升；在投手這邊→控球微升
  const batEdge = momentumEdge(g, bKey);
  const pitEdge = momentumEdge(g, fKey);
  const result = resolvePitch({
    ...
    grade,
    pitcherStats: { control: pitcher.effControl + pitEdge, stuff: pitcher.effStuff, throws: pitcher.throws },
    shift,
    batterMode: mode,
    batX,
    batY,
    ...
    cor: g.cor || 1,
    batterStats: { ...batter, bats: effBats, contact: batter.contact + batEdge },
    strikes: g.strikes,
  });
  ...
  // 氣勢：好結果推向該方（先輕微衰減回中，再疊加本球事件）
  g.momentum = clampMom((g.momentum || 0) * MOM_DECAY);
  const MOM_GAIN = { SINGLE: 8, BUNT_HIT: 8, XBH: 14, HR: 20 };
  if (MOM_GAIN[result.kind]) addMomentum(g, bKey, MOM_GAIN[result.kind]);
  else if (situation.walked || result.kind === 'HBP') addMomentum(g, bKey, 3);
  if (situation.strikeout) addMomentum(g, fKey, 10);
  if (situation.doublePlay) addMomentum(g, fKey, 12);
...
export function actTaunt(room, role) {
  ...
  // 警告階梯（洗牌袋制，每 5 次挑釁保證恰好 1 次；v11 五階）：
  //   第 1 次＝🟨 純警告
  //   第 2 次＝🟥 總教練出場（禁：代打）
  //   第 3 次＝🟥 副教練出場（禁：換投、代打）
  //   第 4 次＝🟥 教練團全數出場（禁：換投、代打、盜壘、牽制）＋送對方一分
  //   第 5 次＝⚫ 裁定比賽，對方勝利
  let warned = false;
  let justEjected = false;
  let justCoachEjected = false;
  let justAllOut = false;
  let forfeited = false;
  if (me.warnings < 5 && drawWarnFromBag(me)) {
    warned = true;
    me.warnings += 1;
    const oppKey = role === 'away' ? 'home' : 'away';
    const opp = g[oppKey];
    if (me.warnings === 1) {
      pushLog(g, `🟨 主審臉色一沉，對 ${short} 板凳提出警告！（累計 1/5——第二次警告總教練就要出場）`);
    } else if (me.warnings === 2) {
      me.ejected = true;
      justEjected = true;
      pushLog(g, `🟥 主審大步走向 ${short} 休息區、比出驅逐手勢！總教練出場——本場無法再下達代打！（警告 2/5）`);
    } else if (me.warnings === 3) {
      me.coachEjected = true;
      justCoachEjected = true;
      pushLog(g, `🟥 主審這次連 ${short} 副教練也一起請出場！本場無法再：換投、代打！（警告 3/5）`);
    } else if (me.warnings === 4) {
      me.allOut = true;
      justAllOut = true;
      opp.score += 1;
      pushLog(g, `🟥 ${short} 教練團全數被驅逐出場！本場無法再：換投、代打、盜壘、牽制——主審並判 ${opp.team.short} 得一分！（警告 4/5）`);
      pushLog(g, `📢 主審厲聲告知 ${short}：再吃一次警告，直接裁定比賽、對方勝利！`);
    } else {
      // 第 5 次：裁定比賽，對方勝利
      forfeited = true;
      pushLog(g, `⚫ ${short} 第五次警告！主審雙手一揮——裁定比賽結束，${opp.team.short} 獲勝！`);
      g.phase = 'gameover';
      g.deadline = null;
      g.winner = opp.team.name;
      g.endReason = `${short} 累計五次警告——主審裁定敗戰`;
      room.status = 'over';
    }
  }
  // 氣勢豪賭：氣勢原本就在你這邊→挑釁壓過對方，氣勢更倒向你；在對方那邊→反噬，激起對手
  const oppKey2 = role === 'away' ? 'home' : 'away';
  let tauntFlow = 'even';
  if (!forfeited) {
    if (momentumFor(g, role) >= 0) {
      addMomentum(g, role, 15);
      tauntFlow = 'amplify';
      pushLog(g, `🔥 ${short} 氣焰高漲，全場氣勢完全倒向這一邊！`);
    } else {
      addMomentum(g, oppKey2, 12);
      tauntFlow = 'backfire';
      pushLog(g, `😰 ${short} 這波挑釁踢到鐵板——反而激起對手，氣勢倒向對面！`);
    }
  }
...
export function actSurrenderRespond(room, role, payload) {
  ...
  if (payload?.accept) {
    ...
    if (kind === 'draw') {
      ...
      // 投降輸一半（新玩法）：對方接受＝兩隊言歸於好、這局判平手，可重開一局再打
      g.winner = null;
      g.endReason = '投降輸一半——兩隊握手言和，這局算平手，重開一局再打吧！';
      pushLog(g, `🤝 ${acceptSide.team.short} 大手一揮：「好啦好啦，投降輸一半！這局平手，重開一局再打！」兩隊握手言和～`);
    }
  } else {
    g.surrender = null;
    pushLog(g, kind === 'draw'
...
export function viewFor(room, role) {
  ...
  const view = {
    ...
    game: {
      ...
      momentum: g.momentum || 0, // 氣勢條（公開）：>0 客隊、<0 主隊
      mvp: g.phase === 'gameover' ? computeMvp(g) : null, // 賽後 MVP 圖卡
      cor: g.cor || 1,
