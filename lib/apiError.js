// 已知業務錯誤代碼白名單。未列出的錯誤一律回傳 INTERNAL，避免下游 lib 的 stack / 連線字串外流。
const SAFE_CODES = new Set([
  'NO_STORE',
  'BAD_INPUT',
  'BAD_CODE',
  'BAD_TEAM',
  'BAD_ACTION',
  'CODE_POOL_BUSY',
  'NOT_FOUND',
  'ROOM_FULL',
  'TEAM_TAKEN',
  'FORBIDDEN',
  'NOT_STARTED',
  'WRONG_PHASE',
  'NOT_YOUR_TURN',
  'INVALID',
  'ALREADY_READY',
  'NO_CHALLENGE',
  'NOT_CHALLENGEABLE',
  'ROOM_BUSY',
  'RATE_LIMITED',
  'LOCK_LOST',
  'NO_RUNNER',
  'PICKOFF_LIMIT',
  // 璀璨寶石
  'GAME_OVER',
  'COIN_PENDING',
  'ALREADY_PICKED',
  'SIDE_TAKEN',
  'CANNOT_AFFORD',
  'RESERVE_FULL',
  'NOT_ENOUGH_GEMS',
  'BAD_TAKE',
  'NO_CARD',
  'WRONG_MODE',
  // 聊天室
  'CHAT_TOO_FAST',
  'MUST_CALL',
  'NOTHING_TO_CALL',
  'OVER_CAP',
  'SUPER_USED',
  'OCCUPIED',
  'FORBIDDEN_33',
  'FORBIDDEN_44',
  'FORBIDDEN_66',
  'DRAW_PENDING',
  'NO_DRAW_OFFER',
  'NOT_ENOUGH_CHIPS',
  'ALREADY_DREW',
  'ALREADY_PLAYED',
  'ALREADY_REVEALED',
  'CLUE_ON_BOARD',
  'SERIES_OVER',
  'REPORT_MISMATCH',
  'EMPTY_MESSAGE',
]);

export function safeErrorCode(err) {
  const msg = typeof err === 'string' ? err : err?.message;
  return SAFE_CODES.has(msg) ? msg : 'INTERNAL';
}

// 依錯誤碼分派 HTTP 狀態與使用者訊息。避免所有例外都被裹上「另一個動作」文案並回 409。
const STATUS_MAP = {
  ROOM_BUSY: { status: 409, message: '房間正在處理另一個動作，請再試一次' },
  LOCK_LOST: { status: 409, message: '房間狀態有更新，請再試一次' },
  NO_STORE: { status: 500, message: '資料庫尚未設定或連線失敗' },
  BAD_CODE: { status: 400, message: '房號格式錯誤' },
  BAD_INPUT: { status: 400 },
  RATE_LIMITED: { status: 429, message: '操作過於頻繁，請稍候再試' },
  INTERNAL: { status: 500, message: '伺服器內部錯誤' },
  // 共用的流程類錯誤：屬於「狀態不對」而非伺服器故障，應回 409/403/404
  NOT_FOUND: { status: 404, message: '找不到這個房號' },
  FORBIDDEN: { status: 403, message: '沒有這個房間的權限' },
  ROOM_FULL: { status: 409, message: '房間已滿' },
  NOT_STARTED: { status: 409, message: '對局尚未開始' },
  WRONG_PHASE: { status: 409, message: '目前不能做這個動作' },
  NOT_YOUR_TURN: { status: 409, message: '還沒輪到你' },
  BAD_ACTION: { status: 400, message: '不支援的動作' },
  CODE_POOL_BUSY: { status: 503, message: '房號暫時擁擠，請再試一次' },
  // 璀璨寶石：都是玩家可以自行修正的狀況，回 409 並附上白話說明
  GAME_OVER: { status: 409, message: '這局已經結束了' },
  COIN_PENDING: { status: 409, message: '還在猜金幣，尚未開始' },
  ALREADY_PICKED: { status: 409, message: '你已經押過了，不能更改' },
  SIDE_TAKEN: { status: 409, message: '這一面已經被對手押走了' },
  CANNOT_AFFORD: { status: 409, message: '寶石不足，買不起這張' },
  RESERVE_FULL: { status: 409, message: '保留卡已有 3 張' },
  NOT_ENOUGH_GEMS: { status: 409, message: '寶石庫存不足' },
  BAD_TAKE: { status: 409, message: '這樣拿不符合規則' },
  NO_CARD: { status: 409, message: '這個位置已經沒有卡了' },
  WRONG_MODE: { status: 409, message: '房號對應的是別種遊戲' },
  CHAT_TOO_FAST: { status: 429, message: '發言太快，稍等一下' },
  EMPTY_MESSAGE: { status: 400, message: '訊息不能是空白' },
  // 17 撲克
  MUST_CALL: { status: 409, message: '有注在身，不能過牌' },
  NOTHING_TO_CALL: { status: 409, message: '沒有需要跟的注' },
  OVER_CAP: { status: 409, message: '超過本階段加注上限' },
  SUPER_USED: { status: 409, message: '這一場的破限加注已經用掉了' },
  NOT_ENOUGH_CHIPS: { status: 409, message: '籌碼不足' },
  ALREADY_DREW: { status: 409, message: '你已經換過牌了' },
  // E 卡
  ALREADY_PLAYED: { status: 409, message: '你已經出過牌了' },
  // 密語連線
  ALREADY_REVEALED: { status: 409, message: '這格已經翻開了' },
  CLUE_ON_BOARD: { status: 409, message: '提示不能用到盤面上出現的字，請換一個詞' },
  // 五子棋
  OCCUPIED: { status: 409, message: '這個點已經有子了' },
  FORBIDDEN_33: { status: 409, message: '三三禁手：這一手會同時做出兩個活三，換一個點' },
  FORBIDDEN_44: { status: 409, message: '四四禁手：這一手會同時做出兩個四，換一個點' },
  FORBIDDEN_66: { status: 409, message: '長連禁手：黑方不可連成六子以上，換一個點' },
  DRAW_PENDING: { status: 409, message: '已經有一個和局要求在等回應了' },
  NO_DRAW_OFFER: { status: 409, message: '目前沒有等你回應的和局要求' },
  // 系列賽
  SERIES_OVER: { status: 409, message: '系列賽已經結束' },
  REPORT_MISMATCH: { status: 409, message: '雙方回報的勝者不一致，請重新確認' },
};

export function errorResponseInfo(err) {
  const code = safeErrorCode(err);
  const spec = STATUS_MAP[code] || { status: 500 };
  return { code, status: spec.status, message: spec.message };
}
