...
function resolveTrick(room) {
  ...
  // ★ 平手：不立刻清掉出牌。先停在「翻牌」狀態，讓雙方都看見兩張市民對撞，
  //   前端播完翻牌動畫後才自動推進（雙方都送到才進下一輪）。
  //   舊版直接清空 r.played，畫面上牌一瞬間就消失、根本看不到開牌。
  r.tieHold = true;
  r.tieReady = { a: false, b: false };
  ec.seq += 1;
}

// 平手翻牌看完 → 推進下一輪。
// 註：此動作為 idempotent——前端會重試，若已不在平手翻牌狀態
// （對方先推進、或本回合已結束）就安靜返回，不可拋錯。
export function actEcardNextTrick(room, role) {
  const ec = room.ec;
  if (!ec) throw new Error('NOT_STARTED');
  const r = ec.r;
  if (ec.phase !== 'play' || !r?.tieHold) return;
  r.tieReady = r.tieReady || { a: false, b: false };
  r.tieReady[role] = true;
  if (r.tieReady.a && r.tieReady.b) advanceTrick(room);
}

// 真正進下一輪：清出牌、先手交替；牌出完仍未分勝負 → 皇帝方勝
function advanceTrick(room) {
  const ec = room.ec;
  const r = ec.r;
  r.tieHold = false;
  r.tieReady = { a: false, b: false };
  ...
  if (r.trick >= MAX_TRICKS || r.hands[r.emperorSide].length === 0) {
...
export function ecardViewFor(room, role) {
  ...
  if (r) {
    ...
    view.r = {
      ...
      bothPlayed,
      // 平手翻牌中：雙方牌面都攤開，前端播完動畫後自動送 ec_next_trick 推進
      tieHold: !!r.tieHold,
      // 誰該出牌
      toPlay: r.played
