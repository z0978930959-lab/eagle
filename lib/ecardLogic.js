...
export function actEcardSticker(room, role, payload) {
  ...
  const now = Date.now();
  ec.stickerAt = ec.stickerAt || {};
  if (now - (ec.stickerAt[role] || 0) < 2000) throw new Error('STICKER_LIMIT'); // 解除次數限制，改 2 秒防連點
  ec.stickerAt[role] = now;
  // 記錄最近一次貼圖（前端放大兩倍、雙方顯示）
  ec.lastSticker = { role, name, ts: now, seq: (ec.seq += 1) };
}
