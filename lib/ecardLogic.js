...
const STICKERS = ['taunt', 'eat', 'serious', 'confident', 'dog', 'cat', 'emperor_face', 'slave_face', 'honest'];
const STICKER_LIMIT = 3; // 每回合限 3 次
...
export function actEcardSticker(room, role, payload) {
  ...
  ec.stickerUse = ec.stickerUse || {};
  const key = `${ec.round}:${role}`;
  ec.stickerUse[key] = (ec.stickerUse[key] || 0) + 1;
  if (ec.stickerUse[key] > STICKER_LIMIT) throw new Error('STICKER_LIMIT');
  // 記錄最近一次貼圖，前端顯示 3 秒
  ec.lastSticker = { role, name, ts: Date.now(), seq: (ec.seq += 1) };
}
...
export function ecardViewFor(room, role) {
  ...
  const view = {
    ...
    iWon: ec.result === role,
    lastSticker: ec.lastSticker || null,
    log: ec.log,
    seq: ec.seq,
  };
