...
// v36：貼圖擴充到 8 張（後端白名單 lib/missionLogic.js 的 MS_STICKERS 要一致）
const MS_STICKERS = [
  ['taunt_1', '柴犬'], ['taunt_2', '哈士奇'], ['taunt_3', '你這隻豬'], ['taunt_4', '要那個幹什麼'],
  ['taunt_5', '做得好'], ['taunt_6', '兄弟，你懂我'], ['taunt_7', '意味深長'], ['taunt_8', '斜眼看你'],
];

/* ---------------- 格子 ---------------- */

function Cell({ cell, canGuess, onGuess, justRevealed, showAnswer, isOver }) {
  const revealed = cell.revealed;
  const answer = cell.answer; // 引導者未翻時可見的答案；終局後雙方都看得到
  const shown = cell.role; // 翻開後的真身分
  // 終局攤牌：沒翻到的格子也把身分畫出來（底色較濃一點，和「真的被翻開」區隔）
  const finalShow = isOver && !revealed && answer;
  const soft = !revealed && answer && (showAnswer || isOver)
    ? (finalShow ? ROLE_COLOR[answer].soft : ROLE_COLOR[answer].soft)
    : 'transparent';
  ...
  return (
    <button
      ...
      style={{
        ...
        ...(finalShow ? { boxShadow: `inset 0 0 0 2px ${ROLE_COLOR[answer].solid}`, opacity: 0.92 } : null),
      }}
    >
      {!revealed && (
        <span className="absolute inset-0 flex flex-col items-center justify-center p-1">
          ...
          {showAnswer && !isOver && answer && (
            <span className="absolute bottom-1 right-1 w-2 h-2 rounded-full" style={{ background: ROLE_COLOR[answer].solid, opacity: 0.85 }} />
          )}
          {finalShow && (
            <>
              <span className="absolute top-0 left-0 right-0 py-0.5 text-center text-[9px] font-bold text-white" style={{ background: ROLE_COLOR[answer].solid }}>
                {ROLE_COLOR[answer].label}
              </span>
              <span className="absolute bottom-0.5 left-0 right-0 text-center text-[8px] tracking-widest text-field-chalk/45">未翻開</span>
            </>
          )}
        </span>
      )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#141a24] via-[#0f1620] to-[#0a0e14] pb-24">
      <div className="max-w-3xl mx-auto px-3 sm:px-5 pt-4">
        ...
        {/* 終局結算（不再蓋住盤面：直接停在這個畫面，下方盤面同時攤開全部答案） */}
        {isOver && (
          <div
            className="rounded-2xl border p-4 mb-4 text-center"
            style={{
              borderColor: v.result === 'win' ? 'rgba(95,207,143,0.45)' : 'rgba(201,72,79,0.45)',
              background: v.result === 'win' ? 'rgba(63,155,104,0.10)' : 'rgba(201,72,79,0.10)',
              animation: 'msResultIn 420ms ease-out both',
            }}
          >
            {v.result === 'bomb' && showFail && (
              <img
                src="/mission/fail.png"
                alt="失敗"
                className="w-full max-w-[220px] mx-auto rounded-xl border-2 border-red-500/40 shadow-xl mb-3"
                style={{ animation: 'memePop 500ms cubic-bezier(.34,1.56,.64,1) both' }}
              />
            )}
            <div className="text-3xl mb-1">{v.result === 'win' ? '🎉' : v.result === 'bomb' ? '💥' : '⏳'}</div>
            <div className="font-display text-xl sm:text-2xl font-black mb-1" style={{ color: v.result === 'win' ? '#5fcf8f' : '#e88' }}>
              {v.result === 'win' ? '挑戰成功！' : v.result === 'bomb' ? '踩到炸彈了…' : '回合用完了'}
            </div>
            <div className="text-xs text-field-chalk/55">
              找到 {v.found}/{v.keyTotal} 個關鍵人物　·　用掉 {v.roundsUsed}/{v.roundsTotal} 回合　·　難度 {v.difficultyLabel}
            </div>
            <div className="text-[11px] text-field-chalk/40 mt-1">
              ↓ 盤面已對雙方公開：<span style={{ color: ROLE_COLOR.key.solid }}>關鍵人物</span>、
              <span style={{ color: ROLE_COLOR.bomb.solid }}>炸彈</span>、
              <span style={{ color: ROLE_COLOR.civ.solid }}>路人</span> 全部揭曉（標「未翻開」的是這局沒被翻到的）
            </div>
            <div className="flex gap-2 justify-center mt-4">
              <button onClick={() => location.reload()} className="px-5 py-2 rounded-xl border border-field-floodlight/60 text-field-floodlight text-sm tracking-widest">再玩一局</button>
        ...
        <div className="grid grid-cols-5 gap-2 mb-4">
          {cells.map((c) => (
            <Cell key={c._idx} cell={c} canGuess={canGuess} onGuess={(i) => act('ms_guess', { index: i })} justRevealed={flipIdx === c._idx} showAnswer={isGuide} isOver={isOver} />
          ))}
        </div>
